...
export default function LoginPage() {
  ...
  const { code } = useParams(); // e.g. "DYPCOE"

  const onSubmit = async (data) => {
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/auth/${code}/login`,
        {
          method: "POST",
      ...
      localStorage.setItem("instituteCode", code);

      router.push(`/${code}/${result.role}/dashboard`);
    } catch (error) {
      setError("root", {
