...
const Profile = () => {
	...
	const fetchProfile = async () => {
		try {
			const res = await fetch(
				`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/hod/profile`,
				{ credentials: "include" }
			);
	...
	const onPasswordSubmit = async (formData) => {
		const promise = fetch(
			`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/hod/profile/change-password`,
			{
				method: "PUT",
				headers: { "Content-Type": "application/json" },
				credentials: "include",
	...
	return (
		<div className="bg-background min-h-screen p-2 space-y-6 text-main">
			...
			<div className="bg-card p-6 rounded-lg shadow">
				<h2 className="text-xl font-semibold mb-4">HOD Profile</h2>
